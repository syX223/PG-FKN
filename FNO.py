...
config = {
    ...
    "lon_grid_num": 32, "lat_grid_num": 32, "depth_grid_num": 32,
...
def load_argo_data():
    argo_files = [
        os.path.join(config["argo_local_path"], "BOA_Argo_2023_01.nc"),
        os.path.join(config["argo_local_path"], "BOA_Argo_2023_02.nc"),
        os.path.join(config["argo_local_path"], "BOA_Argo_2023_03.nc")
    ]
    ...
    if len(argo_clean) == 0:
        ...
        argo_clean = argo_df[
            (~argo_df["temp_scatter_error"].isna()) &
            (~argo_df["salt_scatter_error"].isna()) &
            (argo_df["temp_scatter_error"] < 1.0) &  # 放宽温度误差
            (argo_df["salt_scatter_error"] < 0.5)  # 放宽盐度误差
            ].copy()
        argo_clean = argo_clean.dropna(subset=["lon", "lat", "pres", "temp", "salt"])
    if len(argo_clean) == 0:
    ...
    def forward(self, x):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            out_ft[:, :, :self.modes1, :self.modes2, :self.modes3] = self.compl_mul3d(
                x_ft[:, :, :self.modes1, :self.modes2, :self.modes3], w1)
            out_ft[:, :, -self.modes1:, :self.modes2, :self.modes3] = self.compl_mul3d(
                x_ft[:, :, -self.modes1:, :self.modes2, :self.modes3], w2)
            out_ft[:, :, :self.modes1, -self.modes2:, :self.modes3] = self.compl_mul3d(
                x_ft[:, :, :self.modes1, -self.modes2:, :self.modes3], w3)
            out_ft[:, :, -self.modes1:, -self.modes2:, :self.modes3] = self.compl_mul3d(
                x_ft[:, :, -self.modes1:, -self.modes2:, :self.modes3], w4)
            x = torch.fft.irfftn(out_ft, s=(x.size(-3), x.size(-2), x.size(-1)))
        return x

class Pure_FNO(nn.Module):
    def __init__(self, modes=12, width=32):
        super(Pure_FNO, self).__init__()
        self.modes1 = modes
        self.modes2 = modes
        self.modes3 = modes
        self.width = width
    ...
    def forward(self, x):
        ...
        correction = self.projection(x)
        out = woa_background + correction
        out = torch.clamp(out, 0.0, 1.0)
        return out

def compute_loss_pure(model, grid_input, X_data, y_data):
    ...
    total_loss = config["lambda_temp"] * loss_temp + config["lambda_sal"] * loss_sal
    return total_loss, (loss_temp.item(), loss_sal.item())

def train_pure_fno(model, X_data, y_data, woa_temp_field, woa_sal_field):
    ...
    woa_t_norm = np.clip((woa_temp_field - config["temp_min"]) / (config["temp_max"] - config["temp_min"]), 0, 1)
    woa_s_norm = np.clip((woa_sal_field - config["sal_min"]) / (config["sal_max"] - config["sal_min"]), 0, 1)
    grid_np = np.stack([lon_mesh, lat_mesh, depth_mesh, time_mesh, woa_t_norm, woa_s_norm], axis=-1)
    ...
    print(f"=== Adam Training ({config['epochs_adam']} epochs) ===")
    model.train()
    pbar = tqdm(range(config["epochs_adam"]))
    for epoch in pbar:
        optimizer_adam.zero_grad()
        with amp.autocast():
            loss, _ = compute_loss_pure(model, grid_input, X_data, y_data)
    ...
    print(f"=== L-BFGS Fine-tuning (Low Precision) ===")
    optimizer_lbfgs = torch.optim.LBFGS(
        model.parameters(),
        lr=config["lr_lbfgs"],
        max_iter=config["epochs_lbfgs"],
        tolerance_grad=1e-4,  # 降低梯度容差
        tolerance_change=1e-4,  # 降低更新量容差
...
def invert_temp_sal_fno(model, grid_input):
    ...
    with torch.no_grad():
        out_full = model(grid_input)
        out_np = out_full.squeeze().cpu().numpy()
    t_norm = out_np[..., 0]
    s_norm = out_np[..., 1]
...
if __name__ == "__main__":
    ...
    print("初始化 Pure FNO 模型 (Standard MLP Projection)...")
    model = Pure_FNO(modes=config["fno_modes"], width=config["fno_width"]).to(device)
    ...
    print("\n=== 步骤4/7：模型训练 (Pure Data-Driven Mode - Low Precision) ===")
    model, grid_input = train_pure_fno(model, X_data, y_data, woa_temp, woa_sal)
    print("\n=== 步骤5/7：温盐场反演 ===")
    pred_temp, pred_c, pred_sal = invert_temp_sal_fno(model, grid_input)
    print(f"反演完成：")
    print(f"  温度场形状：{pred_temp.shape}")
    print(f"  盐度场形状：{pred_sal.shape}")
    print("\n=== 步骤6/7：真实数据验证 ===")
